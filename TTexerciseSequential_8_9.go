...
func TouchTypingExerciseSequential(selectedExercise string) { //                     - -
	...
	for {
		...
		case "Sequential_Hira": // 9
			usersGuessOrOptionDirective = prompt_and_Scan_4_RomajiResponse_to_Any_Prompt(aCardS.KeyH) // Hira prompt
			// ... if executed after a Directive is handled, will be prompting from the same card -- it's all good
		}
