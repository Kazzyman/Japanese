...
func TouchTypingExercises12346_10(selectedExercise string) { //       - -
	//
	...
	for {
		...
		*/
		if selectedExercise == "Romaji_PromptD" {
			pick_Difficult_RandomCard_Assign_aCard()
			if isThis_a_cardWeNeedMoreWorkOn > 2 {   // if we have gone without augmenting the random picks with cards we have prev missed ...
				...
				fileHandleBig, err := os.OpenFile("JapLogRPD.txt", os.O_APPEND|os.O_WRONLY|os.O_CREATE, 0600) // append to file
		...
		case "Romaji_Prompt":
			...
			usersGuessOrOptionDirective = semi_Universal_Prompt_Scan_4_HiraResponse_to_RomajiPrompt(aCard.KeyR, selectedExercise)
		...
		case "Romaji_w_Kata_Prompt":
			...
			usersGuessOrOptionDirective = semi_Universal_Prompt_Scan_4_HiraResponse_to_RomajiPrompt(aCard.KeyRK, selectedExercise)
		// ... if executed after a Directive is handled, will be prompting from the same card -- it's all good
		//
		...
		case "Romaji_PromptD":
			// Semi-universal prompt, passing R
			usersGuessOrOptionDirective = semi_Universal_Prompt_Scan_4_HiraResponse_to_RomajiPrompt(aCardD.KeyR, selectedExercise)
			// ... if executed after a Directive is handled, will be prompting from the same card -- it's all good
			//
		}
	...
	outOfForLoop: // this loop is only needed because we want to handel the case of successive directives (tricky)
		...
		for {
			...
			//  v ^ v ^ At this point we know that the usersGuessOrOptionDirective is not a Dir and should be considered to ba a guess,
					...
					switch selectedExercise {
					case "Romaji_Prompt": // 1
						meatOfRomajiExercise(usersGuessOrOptionDirective, selectedExercise, true)
					case "Romaji_w_Kata_Prompt": // 2
						meatOfRomajiKataExercise(usersGuessOrOptionDirective, selectedExercise,true)
					...
					case "Romaji_PromptD": // 10
						meatOfRomajiExerciseD(usersGuessOrOptionDirective, selectedExercise,true)
					}
					break outOfForLoop
