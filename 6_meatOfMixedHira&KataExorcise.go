...
.
*/
func meatOf_Mixed_HiraKataExorcise(in string, skipFlag bool) { // NOTE: we have already been prompted with KeyR
...
func secondTry_meatOf_mixed_HiraKataExorcise(in string) { // NOTE: we have already been prompted with KeyR
	var thisCaseOfAnInHasAlreadyBeenProcessedAbove bool
	thisCaseOfAnInHasAlreadyBeenProcessedAbove = false
	// *******************************************************************************************
	...
	if aCard.KeyR == "zu" {
		...
		if in == "zu" {
			...
			meatOf_Mixed_HiraKataExorcise(in, false)
		}
	}
