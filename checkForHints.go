...
func checkForHints(value string) {
	if value == "あ" { // a ア, maybe a grotesque A
		...
		fmt.Printf(", hint: R-index <- to the 'H' char, ク　no, just no, One angle & they settled for this?  ku,gu \n")
		...
		fmt.Printf(", hint: ring >v to the X char, at least the sa goes to the left, and it looks a lot like se, I'd say  sa-za \n")
		...
		fmt.Printf(", hint: index <-- to the 'C' char, ソ Looks like: しシ, no ノ, n:ん:ン starting w angles `backtrack  so,zo \n")
	} else if value == "た" || value == "だ" { // ta タ, compare to ku ク and ke ケ
		fmt.Printf(", hint: pinky < to the 'Q' char, タ the top left looks like ta:た, more at least than ku:ク, or ke:ケ  ta-da \n")
	} else if value == "ち" || value == "ぢ" { // chi チ, compare to te テ
		fmt.Printf(", hint: pinky on the 'A' char, チ 'some' resemblance - see the middle line as top of backwards c, rarely ji \n")
	} else if value == "つ" || value == "づ" { // tsu ツ, compare to shi シ, she who sleeps low and snores loudly
		fmt.Printf(", hint: pinky to the 'Z' char, tsu-shi: ツシ   shi:シ said   tsu,zu \n")
		...
		fmt.Printf(", hint: ring on the 'S' char, toe:ト is flipped-out. Kicked in the balls, on its head  to,do \n")
	} else if value == "な" { // na ナ, compare to me メ, and nu ヌ
		fmt.Printf(", hint: index < to the 'U' char, ナ is simple, a very simple version of な  Compare: me:メ,  nu:ヌ  ナ \n")
	} else if value == "に" { // ni ニ
		fmt.Printf(", hint: middle < to the 'I' char,  に:ni ニ \n")
	} else if value == "ぬ" { // nu ヌ, compare to me メ, and na ナ
		fmt.Printf(", hint: R-ring<--- to the '1' char, two lines still cross, ヌ flew nu:ヌ, compare to me:メ, & na:ナ \n")
		...
		fmt.Printf(", hint: index on the 'J' char, (a breast pump maybe?) ma マ \n")
		...
		fmt.Printf(", hint: pinky two over ---> to the ']}' chars, now it's an even-fatter-jawed moo-cow, mu ム \n")
	} else if value == "め" { // me メ
		fmt.Printf(", hint: pinky -> slide down to the '/?' chars, cross-off the mess. But that's nothing nu ヌ me:メ\n")
	} else if value == "も" { // mo モ
		fmt.Printf(", hint: middle <v to the 'M' char, way to hang to the right; you go Joe!  mo モ \n")
	} else if value == "ら" { // ra ラ
		fmt.Printf(", hint: ring ^ to the 'O' char, Similar ra:la:ラ:ら \n")
		...
		fmt.Printf(", hint: pinky on the ';+' chars,  レ, れ; しshe-it   re:レ, more, or less, a ru:ル - しシshe-said \n")
	} else if value == "ろ" { // ro ロ
		fmt.Printf(", hint: pinky ---> long slide to the '_\\' chars, it had always been the way to go: a square 'O' for ro ロ \n")
		...
		fmt.Printf(", hint: ring > shifted '0' char,  wo:ヲ,  Compare: wa:ワ  ウ:u:ウ　フ:fu:hu:フ  wo:ヲ テ:te  ra:ラ \n")
	} else if value == "ん" { // nh ン, compare to so ソ
		fmt.Printf(", hint: 'Y' char, pointing at last remaining bent stroke of the hira char nh ン, Compare: so:ソ \n")
	} else {
		fmt.Printf("\n")
	}
}
